...
let input3 = `threerznlrhtkjp23mtflmbrzq395three
9sevenvlttm
3twochzbv
...
let input2 = `fourtwo6zjtjmtdkfzmltxdhltdzrtqp
two1nine
eightwothree
abcone2threexyz
xtwone3four
...
array.forEach((el) => {
  ...
  numberArray.forEach((number) => {
    let index = el.indexOf(number.string);
    if (index != -1) {
      if (index <= min) {
  ...
  let elRev = el.split("").reverse().join("");
  min = el.length;
  numberArray.forEach((number) => {
    let revStr = number.string.split("").reverse().join("");
    // console.log(revStr);
    let index = elRev.indexOf(revStr);
    if (index != -1) {
      if (index <= min) {
        // console.log(index + ", " + number.value);
        min = index;
