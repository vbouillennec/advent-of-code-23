...
class Hand {
	...
	isFiveOfAKind() {
		return this.cards.filter(card => card.value === this.cards[0].value).length === 5;
	}

	isFourOfAKind() {
		return this.cards.find((card) => this.cards.filter(c => c.value === card.value).length === 4) !== undefined;
	}
	...
	isFullHouse() {
		...
		this.cards.forEach(card => {
			if (this.cards.filter(c => c.value === card.value).length === 3) {
				if(threeOfAKind === '') {
					threeOfAKind = card.value;
				}
			}
			if (this.cards.filter(c => c.value === card.value).length === 2) {
				if(pair === '') {
					pair = card.value;
				}
			}
	...
	isThreeOfAKind() {
		return this.cards.some(card => this.cards.filter(c => c.value === card.value).length === 3);
	}
	...
	isOnePair() {
		return this.cards.some(card => this.cards.filter(c => c.value === card.value).length === 2);
	}
	...
	toString() {
		return `\nHand ID: ${this.id}, ${this.cardsString}, bid: ${this.bid}, strength: ${this.type.strength}`;
	}
}
