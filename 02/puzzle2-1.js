...
games.forEach((game) => {
  ...
  let cubeSets = game.split(":")[1].split(";");
  let isOver = false;
  cubeSets.forEach((set) => {
    let cubeSet = set.trim();
    let cubeArr = cubeSet.split(", ");
    cubeArr.forEach((cubeStr) => {
      cubeObj = cubeStr.split(" ");
      const cubeNb = cubeObj[0];
      const cubeColor = cubeObj[1];

      // console.log(`${cubeColor}: ${cubeNb}`);
      if (+cubeNb > min[cubeColor]) {
        min[cubeColor] = +cubeNb;
        // console.log("gameId: ", gameId);
      }
    });
  });
  console.log(min);
  addition += min.red * min.green * min.blue;
  console.log(min.red * min.green * min.blue);
  // process.exit();
});
