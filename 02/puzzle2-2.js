...
games.forEach((game) => {
  let gameId = +game.match(/Game ([0-9]*)/)[1];
  addition += getMaxColor("red", game) * getMaxColor("green", game) * getMaxColor("blue", game);
});

console.log(addition);

function getMaxColor(colorName, game) {
  let regex = new RegExp("([0-9]*) " + colorName, "g");
  let colorCubes = [...game.matchAll(regex)];
  let colorMap = colorCubes.map((color) => +color[1]);
  return Math.max(...colorMap);
}
